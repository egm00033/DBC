...
int **leerImagenMapa(char *nome,long int &ancho,long int &alto){
	...
	for (i = 0; i < ancho*alto; i++) {
		char red;
		fread (&red, sizeof (char), 1, fpin);
		vector[i]=(uint8_t) red & 0xff;
			//printf("%i=%i\n",i,vector[i] & 0xff );
	}
	int anchoMatriz=ancho;
	int altoMatriz=alto;
	//reserva de memoria para la matriz
	matriz =(int **) calloc(altoMatriz,sizeof(int *));
	for (int i = 0; i < altoMatriz; i++){
	...
	for (i = 0; i < (ancho) * (alto); i++) {
			...
			matriz[fila][col]=vector[i];//leer en unsigned int de 1Byte
			col+=1;
