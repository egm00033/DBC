...
shaderGPU2_2::shaderGPU2_2(void) : shader(2,"kernel/GPU_2_2_memoria.cl")
{

}

//Calcula la sumatoria de N para todas las s
void shaderGPU2_2::CalcularN(unsigned char *img3,float *NdeS, int M, int tamListaS,int *listaS){
	...
	if(ret==0){
		...
		if(ret==-54){
			...
			ret = clEnqueueReadBuffer(command_queue, salida_mem_obj, CL_TRUE, 0, tamListaS * sizeof(float), NdeS, 0, NULL, NULL);
...
shaderGPU2_2::~shaderGPU2_2(void)
{
