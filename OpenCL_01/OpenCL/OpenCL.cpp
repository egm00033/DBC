...
void calcularMinMax(int s, int M, cl_device_id device_id, char *shader, size_t source_size, int *entrada){
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	LIST_SIZE = clGetPlatformIDs(0, NULL, &ret_num_platforms);
	cl_platform_id *platforms = NULL;
	platforms = (cl_platform_id*)malloc(ret_num_platforms*sizeof(cl_platform_id));

	cl_int ret = clGetPlatformIDs(ret_num_platforms, platforms, NULL);
	printf("respuesta de la linea %d es %d\n", __LINE__, ret);
	...
	while(s<=M/2){
		calcularMinMax(s, M, device_id, shader, source_size, entrada);
	}
	free(maximos);
	free(minimos);
	free(sumn);
